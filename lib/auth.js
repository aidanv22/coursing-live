...
export async function getCurrentCompany() {
  ...
  // If this account's email matches PLATFORM_ADMIN_EMAIL, ensure it's
  // flagged as the platform admin. Checked on every load rather than only
  // at signup, so setting/changing the env var takes effect immediately
  // without needing a manual database update.
  const adminEmail = process.env.PLATFORM_ADMIN_EMAIL?.toLowerCase();
  if (adminEmail && company.email === adminEmail && !company.is_platform_admin) {
    const updated = await sql`
      UPDATE companies SET is_platform_admin = TRUE WHERE id = ${company.id} RETURNING *
    `;
    company = updated[0];
  }

  return ensureCompanyIdentifiers(company);
}
