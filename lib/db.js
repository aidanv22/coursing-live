...
export async function ensureSchema() {
  ...
  schemaReady = (async () => {
    ...
    // Each company now needs its own dedicated, individually-verified Twilio
    // number (required by the ISV/Secondary Customer Profile structure —
    // one shared number across businesses doesn't satisfy carrier
    // verification, per Twilio error 30474). NULL until you've bought and
    // verified a number for that specific business.
    await sql`ALTER TABLE companies ADD COLUMN IF NOT EXISTS twilio_phone_number TEXT`;
