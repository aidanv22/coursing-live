...
export async function listTollFreeVerifications() {
  // Telnyx's list endpoint requires `page` and `page_size` as query
  // params — omitting them causes a 400 "A required parameter was
  // missing" error. 50 is a reasonable page size for the handful of
  // client companies you'll realistically have on one account.
  const data = await telnyxFetch('?page=1&page_size=50');
